...
import com.rm.app.ui.tool.RMAppEditorRuler;


public class RMAppDiagramPane extends JScrollPane implements Printable {
	 ...
	 */
	// protected RMAppDiagram diagram;
	/**
	 ...
	 */
	 protected RMAppEditorRuler verticalRuler, horizontalRuler;
	/**
	 ...
	 */
	public static String PROPERTY_METRIC = "metric", PROPERTY_PAGEVISIBLE = "pageVisible",
			PROPERTY_BACKGROUNDIMAGE = "backgroundImage", PROPERTY_RULERSVISIBLE = "rulersVisible",
			PROPERTY_PAGEFORMAT = "pageFormat", PROPERTY_AUTOSCALEPOLICY = "autoScalePolicy",
			PROPERTY_PAGESCALE = "pageScale";

	/**
	 * Constructs a new graph pane for the specified diagram and graph.
	 */
	public RMAppDiagramPane(RMGraph graph) {
		super();
	...
	public PropertyChangeListener createAutoScaleResetListener() {
		return new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				if (evt.getPropertyName().equals(RMGraph.SCALE_PROPERTY)
	...
	protected void createRulers() {
		horizontalRuler = new RMAppEditorRuler(graph,
			RMAppEditorRuler.ORIENTATION_HORIZONTAL);
		verticalRuler = new RMAppEditorRuler(graph,
			RMAppEditorRuler.ORIENTATION_VERTICAL);
	}
	...
	public BufferedImage getImage(Color bg, int inset) {
		...
		if (bounds != null) {
			...
			BufferedImage img = new BufferedImage((int) bounds.getWidth() + 2 * inset, (int) bounds.getHeight() + 2
					* inset, (bg != null) ? BufferedImage.TYPE_INT_RGB : BufferedImage.TYPE_INT_ARGB);
	 ...
	 */
	// public RMAppDiagram getDiagram() {
	// return diagram;
	// }
	/**
	 ...
	 */
	// public RMAppEditorRuler getVerticalRuler() {
	...
	// */
	// public RMAppEditorRuler getHorizontalRuler() {
	...
	protected void updateMinimumSize() {
		if (isPageVisible() && pageFormat != null) {
			...
			Dimension size = (bounds != null) ? new Dimension((int) (bounds.getX() + bounds.getWidth()), (int) (bounds
					.getY() + bounds.getHeight())) : new Dimension(1, 1);
			int w = (int) (pageFormat.getWidth() * pageScale);
			int h = (int) (pageFormat.getHeight() * pageScale);
			 int cols = (int) Math.ceil((double) (size.width - 5) / (double)
			 w);
			 int rows = (int) Math.ceil((double) (size.height - 5) / (double)
			 h);
//			int cols = (int) Math.ceil((double) (size.width ) / (double) w);
//			int rows = (int) Math.ceil((double) (size.height ) / (double) h);
			size = new Dimension(Math.max(cols, 1) * w + 5, Math.max(rows, 1) * h + 5);
			graph.setMinimumSize(size);

			// Updates the active region in the rulers.
			 horizontalRuler.setActiveOffset((int)
			 (pageFormat.getImageableX()));
			 verticalRuler.setActiveOffset((int)
			 (pageFormat.getImageableY()));
			 horizontalRuler.setActiveLength((int)
			 (pageFormat.getImageableWidth()));
			 verticalRuler.setActiveLength((int)
	...
	protected double computeWindowScale(int border) {
		...
		if (p != null) {
			return Math.min((double) size.getWidth() / (p.getX() + p.getWidth() + border), (double) size.getHeight()
	...
	protected double computePageScale() {
		...
		if (p != null && (p.getWidth() != 0 || p.getHeight() != 0)) {
			return Math.min((double) size.getWidth() / (double) p.getWidth(), (double) size.getHeight()
					/ (double) p.getHeight());
		}
		return 0;
	}
	 ...
	 */
	public static RMAppDiagramPane getParentDiagramPane(Component component) {
		while (component != null) {
			if (component instanceof RMAppDiagramPane)
				return (RMAppDiagramPane) component;
