...
public class RMEvHatAdjDialogBuilder extends RMFlowAdjustDialogBuilder {

    public RMEvHatAdjDialogBuilder(RFlowProperties properties) {
		super(properties);
	}
	...
	final RHATComponent rstudent = (RHATComponent) rcmpt;
	...
	JLabel deLabel = new JLabel("  HAT value ");
	...
	for (int i = 0, k = outlierList.size(); i < k; i++) {
	    RHATComponent.Outlier out = (RHATComponent.Outlier) outlierList.get(i);
...
//	extraPanel.add(checkBox);
	...
	protected ButtonClickListener(RComponent rcmpt) {
	    this.rcmpt = (RHATComponent) rcmpt;
	}

	public void actionPerformed(ActionEvent e) {

	    Iterator areaIterator = inputSet.iterator();
	    List<RHATComponent.Outlier> outList = rcmpt.getOutlierList();
...
//		    } else {
			for (RHATComponent.Outlier out : outList) {
