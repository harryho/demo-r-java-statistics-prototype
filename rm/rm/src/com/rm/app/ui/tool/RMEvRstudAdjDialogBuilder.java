...
public class RMEvRstudAdjDialogBuilder extends RMFlowAdjustDialogBuilder {

    public RMEvRstudAdjDialogBuilder(RFlowProperties properties) {
		super(properties);
	}
	...
	final RRstudentComponent rstudent = (RRstudentComponent) rcmpt;
	...
	for (int i = 0, k = outlierList.size(); i < k; i++) {
	    RRstudentComponent.Outlier out = (RRstudentComponent.Outlier) outlierList.get(i);
...
//	extraPanel.add(checkBox);
	...
	RRstudentComponent rcmpt;

	protected ButtonClickListener(RComponent rcmpt) {
	    this.rcmpt = (RRstudentComponent) rcmpt;
	}

	public void actionPerformed(ActionEvent e) {

	    Iterator areaIterator = inputSet.iterator();
	    List<RRstudentComponent.Outlier> outList = rcmpt.getOutlierList();
...
//		    } else {
			for (RRstudentComponent.Outlier out : outList) {
