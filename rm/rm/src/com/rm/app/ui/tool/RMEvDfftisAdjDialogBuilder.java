...
public class RMEvDfftisAdjDialogBuilder extends RMFlowAdjustDialogBuilder {

    public RMEvDfftisAdjDialogBuilder(RFlowProperties properties) {
		super(properties);
	}
	...
	final RDffitsComponent rstudent = (RDffitsComponent) rcmpt;
	...
	JLabel deLabel = new JLabel("  DFFITS ");
	...
	for (int i = 0, k = outlierList.size(); i < k; i++) {
	    RDffitsComponent.Outlier out = (RDffitsComponent.Outlier) outlierList.get(i);
...
//	extraPanel.add(checkBox);
	...
	RDffitsComponent rcmpt;

	protected ButtonClickListener(RComponent rcmpt) {
	    this.rcmpt = (RDffitsComponent) rcmpt;
	}

	public void actionPerformed(ActionEvent e) {

	    Iterator areaIterator = inputSet.iterator();
	    List<RDffitsComponent.Outlier> outList = rcmpt.getOutlierList();
...
//		    } else {
			for (RDffitsComponent.Outlier out : outList) {
