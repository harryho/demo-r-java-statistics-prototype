...
public class RGLSComponent implements RESComponent {

    private String key;
    private RFlowProperties properties;

    public RGLSComponent(String key, RFlowProperties properties) {
	this.key = key;
	...
	tmp.append("load('nlme')\n");
	tmp.append(properties.getSummaryParam() + " <- gls").append("(");
	...
	if (properties.getIndepend_var() == null || properties.getIndepend_var().size() < 1) {
	    throw new RMException("Independent variable is not correct !");
	}
	if (properties.getDepend_var() == null || properties.getDepend_var().size() < 1) {
	    throw new RMException("Dependent variable is not correct !");
	}
	return true;
    }
